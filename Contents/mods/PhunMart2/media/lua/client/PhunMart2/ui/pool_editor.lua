...
local profileName = "PhunMartUIPoolEditor"
PhunMartUIPoolEditor = ISCollapsableWindowJoypad:derive(profileName);
local UI = PhunMartUIPoolEditor
local instances = {}
...
function UI.OnOpenPanel(playerObj, pool)
    ...
    if not instance then
        ...
        ISLayoutManager.RegisterWindow(profileName, PhunMartUIPoolEditor, instance)
    end
...
function UI:createChildren()
    ...
    self.controls.items = Core.ui.poolEditorItems:new(0, 100, w, self.controls.tabPanel.height - th, {
        player = self.player
    });
...
function UI:onOK()
    ...
    for _, v in ipairs(self.controls.items.categories) do
        if v.item.selected then
            selected.categories[v.item.label] = true
        end
    end
    for _, v in ipairs(self.controls.items.exclusions) do
        if v.selected then
            selected.exclude[v.type] = true
        end
    end
    for _, v in ipairs(self.controls.items.inclusions) do
        if v.selected then
