...
local profileName = "PhunMartUIAdminLocations"
PhunMartUIAdminLocations = ISPanelJoypad:derive(profileName);
local UI = PhunMartUIAdminLocations
...
function UI:createChildren()
    ...
    self.list = ISScrollingListBox:new(x, y, self:getWidth(), self.height - HEADER_HGT - 45);
...
function UI:drawDatas(y, item, alt)
    ...
    self:drawText(item.item.zone.title, xoffset, y + 4, 1, 1, 1, a, self.font);
    self:clearStencilRect()

    local value = item.item.location.x .. ", " .. item.item.location.y .. " (" ..
                      PhunTools:formatWholeNumber(item.item.distance) .. "m)"
