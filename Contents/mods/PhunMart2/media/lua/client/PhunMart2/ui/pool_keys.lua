...
local Core = PhunMart
local profileName = "PhunMartUIAdminPoolKeys"
PhunMartUIAdminPoolKeys = ISPanelJoypad:derive(profileName);
...
function UI:setData(data, itemLookup)
    ...
    for _, v in pairs(data.keys) do
        ...
        local texture = Core:getTextureFromItem(i.display)
        local lbl = Core:getLabelFromItem(i.display)
